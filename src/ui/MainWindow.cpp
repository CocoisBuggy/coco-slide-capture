...
void MainWindow::draw_live_view(GtkDrawingArea *area, cairo_t *cr, int width, int height, gpointer data) {
...
gboolean MainWindow::on_key_press(GtkEventControllerKey *controller, guint keyval, guint keycode, GdkModifierType state, gpointer user_data) {
...
void MainWindow::on_directory_changed(GtkEditable *editable, gpointer user_data) {
    MainWindow* self = static_cast<MainWindow*>(user_data);
    const char *text = gtk_editable_get_text(editable);
    self->active_directory = text;
    std::cout << "Active directory set to: " << self->active_directory << std::endl;
}
...
MainWindow::MainWindow(GtkApplication* app) : camMgr(nullptr), renderer(nullptr) {
    // Create window
    ...
    g_signal_connect(dir_entry, "changed", G_CALLBACK(on_directory_changed), this);
    ...
    g_signal_connect(calendar, "day-selected", G_CALLBACK(on_date_selected), this);
    ...
    if (camMgr->initialize() && camMgr->connectCamera() && camMgr->startLiveView()) {
        ...
        gtk_drawing_area_set_draw_func(GTK_DRAWING_AREA(live_view), draw_live_view, this, NULL);
        renderer->start();
